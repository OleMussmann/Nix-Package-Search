...
filter_caches() {
    ...
    #          "*[not a '.'].[not a '.'].PACKAGE_NAME\x1B\[0m"
    #        and replace it with
    #          "OPTIONAL_REGISTRY_NAME[no space]PACKAGE_NAME"
    #      - FILTER_SECOND_LINE
    #        remove formatting, delete
    #          "  \x1B\[32;1m\x1B\[0m" from the start of the line
    #      - resulting in
    ...
    ADD_VERSION_IF_MISSING="/^*/s/\(\x1B\[0m\)$/\1 (-)/"
    FILTER_VERSION="/^*/s/(\(.*\))/\1/"
    FILTER_PACKAGE_NAME="s/^\*[^\.]*\.[^\.]*\.\(.*\)\x1B\[0m"
    FILTER_PACKAGE_NAME+="/$REGISTRY_NAME\1/"

    FILTER_SECOND_LINE="s/^  \x1B\[32;1m\x1B\[0m//"

    sed -e "$ADD_VERSION_IF_MISSING" | \
    sed -e "$FILTER_VERSION" | \
    sed -e "$FILTER_PACKAGE_NAME" | \
    sed -e "$FILTER_SECOND_LINE" | \
...
refresh() {
    ...
    if [ "$EXPERIMENTAL" == "true" ]; then
        ...
        filter_caches "" < "/tmp/nps/raw_caches/$SYSTEM_FLAKE" \
            > "/tmp/nps/$EXPERIMENTAL_CACHE_FILE"
	...
	echo
        ...
        if CACHE="$(nix-env -qaP --description)"; then
